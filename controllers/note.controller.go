...
import (
	...
	"github.com/ramdeoangh/goswagger/models"
	"gorm.io/gorm"
)
...
func FindNotesByDate(c *fiber.Ctx) error {

	var searchPayload *models.SearchNoteSchema

	if err := c.BodyParser(&searchPayload); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"status": "fail", "message": err.Error()})
	}

	errors := models.ValidateStruct(searchPayload)
	if errors != nil {
		return c.Status(fiber.StatusBadRequest).JSON(errors)

	}

	searchNote := models.SearchNoteSchema{
		StartDate: searchPayload.StartDate,
		EndDate:   searchPayload.EndDate,
	}

	var notes []models.Note
	results := initializers.DB.Where("DATE_FORMAT(created_at, '%Y-%m-%d') between ? and ?", searchNote.StartDate, searchNote.EndDate).Find(&notes)
	if results.Error != nil {
		return c.Status(fiber.StatusBadGateway).JSON(fiber.Map{"status": "error", "message": results.Error})
	}
